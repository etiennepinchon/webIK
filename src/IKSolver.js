...
export default class IKSolver {
  ...
  initiate(root) {
    if (this.onPreInitiate != null) this.onPreInitiate();

    if (root == null) console.error("Initiating IKSolver with null root Transform.");
    this.root = root;
    ...
    if (this.onPostInitiate != null) this.onPostInitiate();
  }
  ...
  update() {
    if (this.onPreUpdate != null) this.onPreUpdate();
    ...
    if (this.onPostUpdate != null) this.onPostUpdate();
  }
  ...
  static containsDuplicateBone(bones) {
  ...
  static preSolveBones(bones) {
    ...
    for (let i = 0; i < bones.length; i++) {
      bones[i].solverPosition = bones[i].transform.position.clone();
      bones[i].solverRotation = bones[i].transform.quaternion.clone();
