...
export default class IKSolverVR extends IKSolver {
  ...
  // Guesses the hand bones orientations ('Wrist To Palm Axis' and "Palm To Thumb Axis" of the arms) based on the provided references. if onlyIfZero is true, will only guess an orientation axis if it is Vector3.zero.
	guessHandOrientations(references, onlyIfZero) {
		if (!references.isFilled) {
  ...
  /// Adds position offset to a body part. Position offsets add to the targets in VRIK.
	addPositionOffset(positionOffset, value) {
	...
	addRotationOffset(rotationOffset, value) {
		this.addRotationOffsetFromQuaternion(rotationOffset, Quaternion.Euler(value));
	}

	// Adds rotation offset to a body part. Rotation offsets add to the targets in VRIK
	addRotationOffsetFromQuaternion(rotationOffset, value) {
		switch(rotationOffset) {
  		case RotationOffset.Pelvis: this.spine.pelvisRotationOffset = value.multiply(this.spine.pelvisRotationOffset); return;
  		case RotationOffset.Chest: this.spine.chestRotationOffset = value.multiply(this.spine.chestRotationOffset); return;
  		case RotationOffset.Head: this.spine.headRotationOffset = value.multiply(this.spine.headRotationOffset); return;
		}
	}

  // Call this in each Update if your avatar is standing on a moving platform
	addPlatformMotion(deltaPosition, deltaRotation, platformPivot) {
		...
		this.spine.faceDirection = deltaRotation.clone().multiply(spine.faceDirection);
	}
  ...
  // Resets all tweens, blendings and lerps. Call this after you have teleported the character.
	...
	storeDefaultLocalState() {
        ...
        this.defaultLocalRotations[i - 1] = this.solverTransforms[i].quaternion.clone();
		}
	}

  // Reset transformations.
	fixTransforms() {
		this.solverTransforms[1].position.setFromVector3(this.defaultPelvisLocalPosition);
		for (let i = 1; i < this.solverTransforms.length; i++) {
		    if (this.solverTransforms[i]) {
          ...
          this.solverTransforms[i].applyQuaternion( new Quaternion(qua.x, qua.y, qua.z, qua.w) );
        }
		}
	}
	...
	isValid() {
		...
		if (!this.leftArm.wristToPalmAxis.length()) {
		...
		if (!this.rightArm.wristToPalmAxis.length()) {
		...
		if (!this.leftArm.palmToThumbAxis.length()) {
		...
		if (!this.rightArm.palmToThumbAxis.length()) {
  ...
  getNormal(transforms) {
		const normal = new Vector3();
		const centroid = new Vector3();

		for (let i = 0; i < transforms.length; i++) {
			centroid.add(transforms[i].position);
		}
		centroid.divide(transforms.length);

		for (let i = 0; i < transforms.length - 1; i++) {
			normal.add( transforms[i].position.clone().sub(centroid)
                    .cross( transforms[i + 1].position.clone().sub(centroid) )
                    .normalize() );
		}
  ...
  guessWristToPalmAxis(hand, forearm) {
		const toForearm = forearm.position.clone().sub(hand.position);
		let axis = AxisTools.toVector3(AxisTools.getAxisToDirection(hand, toForearm));
		if (toForearm.dot(hand.quaternion.clone().multiply(axis)) > 0) axis = axis.negate();
		return axis;
	}

	guessPalmToThumbAxis(hand, forearm) {
		...
		for (let i = 0; i < hand.children.length; i++) {
			let sqrMag = hand.children[i].position.clone().sub(hand.position).lengthSq();
		...
		let handNormal = hand.position.clone().sub(forearm.position).cross( hand.children[thumbIndex].position.clone().sub(hand.position) );
		let toThumb = handNormal.cross(hand.position.clone().sub(forearm.position));
		let axis = AxisTools.toVector3(AxisTools.getAxisToDirection(hand, toThumb));
		if (toThumb.dot(hand.quaternion.clone().multiply(axis)) < 0) axis = axis.negate();
		return axis;
	}
	...
	updateSolverTransforms() {
		for (let i = 0; i < this.solverTransforms.length; i++) {
			if (this.solverTransforms[i] != null) {
				this.readPositions[i] = this.solverTransforms[i].position.clone();
				this.readRotations[i] = this.solverTransforms[i].quaternion.clone();
			}
		}
	}
  ...
  writeTransforms() {
		for (let i = 0; i < this.solverTransforms.length; i++) {
			if (this.solverTransforms[i]) {
				if (i < 2 && i != 0) {
          let pos = V3Tools.lerp(this.solverTransforms[i].position, this.getPosition(i), this.IKPositionWeight);
					this.solverTransforms[i].position.set( pos.x, pos.y, pos.z );
        }
        ...
        let qua = QuaTools.lerp(this.solverTransforms[i].quaternion, this.getRotation(i), this.IKPositionWeight);

        if (qua.w == undefined || isNaN(qua.w)) {
          console.error('corrupt data qua', this.solverTransforms[i].quaternion, this.getRotation(i), this.IKPositionWeight)
          throw new Error()
        }
  ...
  read(positions, rotations, hasChest, hasNeck, hasShoulders, hasToes) {
		...
		if (!this.initiated) {
			...
			this.raycastOriginPelvis = this.spine.pelvis.readPosition;
			this.spine.faceDirection = this.readRotations[0].clone().multiply(new Vector3(0, 0, 1));
		}
	}
  ...
  solve() {
        ...
        let rootUp = this.root.quaternion.clone().multiply( Vector3.up );
				...
				this.leftLeg.footPositionOffset.add(leftFootPosition.clone().sub(this.leftLeg.lastBone.solverPosition).multiplyScalar(this.IKPositionWeight).multiplyScalar( (1 - this.leftLeg.positionWeight) * this.locomotion.weight ));
				this.rightLeg.footPositionOffset.add(rightFootPosition.clone().sub(this.rightLeg.lastBone.solverPosition).multiplyScalar(this.IKPositionWeight).multiplyScalar( (1 - this.rightLeg.positionWeight) * this.locomotion.weight ));
				...
				let rotationOffsetLeft = QuaTools.fromToRotation(this.leftLeg.lastBone.solverRotation.clone(), leftFootRotation.clone());
				let rotationOffsetRight = QuaTools.fromToRotation(this.rightLeg.lastBone.solverRotation.clone(), rightFootRotation.clone());
        ...
        this.leftLeg.footRotationOffset = rotationOffsetLeft.clone().multiply( this.leftLeg.footRotationOffset.clone() );
				this.rightLeg.footRotationOffset = rotationOffsetRight.clone().multiply( this.rightLeg.footRotationOffset.clone() );
				...
				let p = this.rootBone.solverPosition.clone().add( this.rootVelocity.clone().multiplyScalar( Time.deltaTime * 2 * this.locomotion.weight) );
    ...
    getPosition(index) {
		...
		getPelvisOffset() {
			...
			let sampledOrigin = raycastOriginPelvis;
			sampledOrigin.y = spine.pelvis.solverPosition.y;
			let origin = spine.pelvis.readPosition;
			origin.y = spine.pelvis.solverPosition.y;
			let direction = origin.clone().sub( sampledOrigin );
			//RaycastHit hit;
			...
			let position = this.spine.pelvis.solverPosition;
      ...
      //
			...
			this.lastOffset = this.lastOffset.lerp(position.clone().sub(spine.pelvis.solverPosition), Time.deltaTime * 15);
