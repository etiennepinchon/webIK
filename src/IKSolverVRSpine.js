...
export default class Spine extends BodyPart {
	...
	preSolve() {
		...
		if (this.headTarget) {
			...
			let rot = this.headTarget.getWorldQuaternion();
			this.IKRotationHead = new Quaternion(rot.x, rot.y, rot.z, rot.w);
		}
		...
		if (this.pelvisTarget) {
			...
			let rot = this.pelvisTarget.getWorldQuaternion();
			this.IKRotationPelvis = new Quaternion(rot.x, rot.y, rot.z, rot.w);
		}
