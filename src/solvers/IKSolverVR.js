...
export default class IKSolverVR extends IKSolver {
  ...
  guessWristToPalmAxis(/*Transform*/hand, /*Transform*/forearm) {
    ...
    let rot = hand.getUnityWorldQuaternion();
    ...
    let handQua = hand.getUnityWorldQuaternion();
    ...
    window._scene.updateMatrixWorld();
		...
		for (let i = 0; i < this.solverTransforms.length; i++) {
			if (this.solverTransforms[i]) {
				...
				this.readRotations[i] = this.solverTransforms[i].getUnityWorldQuaternion();
			}
		}
	}
  ...
  writeTransforms() {
    window._scene.updateMatrixWorld();
        ...
        let rot = this.solverTransforms[i].getUnityWorldQuaternion();
        ...
        if (i != 0) {
          ...
          this.solverTransforms[i].setUnityWorldQuaternion( qua );
